...
class Graph():
    ...
    def get_general_sink(self):
        self.build_component_graph()
        self.topological_sort()
        bfs_visited = set()
        bfs_queue = [self.sorted_nodes[0]]
        while bfs_queue:
            u = bfs_queue[0]
            bfs_queue.pop(0)
            if u in bfs_visited:
            ...
            for v in self.components_graph[u]:
                bfs_queue.append(v)
        if len(bfs_visited) == len(self.sorted_nodes):
            return self.components[self.sorted_nodes[0]][0]
        else:
            return -1
...
def main():
    ...
    for edge_list in edge_list_array:
        ...
        rv.append(graph.get_general_sink())
